...
def load_data():
    ...
    return collect(include_slow=False)


@st.cache_data(ttl=3600)
def _load_onchain_slow_raw():
    # bitcoin-data.com: 시간당 10회 제한 → 1시간 캐시로 묶어 호출 최소화.
    return fetch_onchain_slow()


def load_onchain_slow():
    """느린 온체인 지표를 1시간 캐시로 받되, 비어 있으면(429 등) 마지막 정상값을 유지."""
    data = _load_onchain_slow_raw()
    if (data.get("valuation") or data.get("advanced")):
        st.session_state["onchain_slow_last"] = data
        return data
    return st.session_state.get("onchain_slow_last", data)
...
with col_r2:
    if st.button("🔄 새로고침", use_container_width=True):
        # 빠른 캐시만 비움. 느린 온체인 지표(_load_onchain_slow_raw)는 시간당 10회
        # 제한이 있어 보존한다 (1시간마다 자동 갱신).
        for _f in (load_data, load_premiums, load_long_short, load_cex_reserves,
                   load_lightning, load_stablecoins, load_defi_tvl, load_derivatives,
                   load_trending, load_top_coins, load_liquidations):
            try:
                _f.clear()
            except Exception:
                pass
...
try:
    with st.spinner("📡 데이터 수집 중... (실시간 소스에서 가져오는 중)"):
        report = load_data()
...
# 느린 온체인 지표(MVRV/NUPL/실현가/SOPR/Puell 등)를 별도 캐시로 병합
try:
    slow = load_onchain_slow()
    if slow.get("valuation"):
        report["valuation"] = slow["valuation"]
    if slow.get("advanced"):
        report["advanced"] = slow["advanced"]
except Exception:
    pass
...
if buy_sig:
    ...
    # 메인 카드
    st.markdown(f"""
    <div style="background:{bs_bg};border:2px solid {bs_border};border-radius:20px;
                padding:2rem;margin:1rem 0;">
        <div style="text-align:center;margin-bottom:1.5rem;">
            <div style="color:#656d76;font-size:0.85rem;font-weight:600;margin-bottom:0.3rem;">
                🎯 온체인 데이터 기반 매수 적기 판단
            </div>
            ...
            <div style="font-size:1rem;color:#4b5563;max-width:600px;margin:0.5rem auto;
                        line-height:1.5;">
                {buy_sig["summary"]}
            </div>
        </div>
...
if has_valuation:
    ...
    st.markdown(
        ...
        'bitcoin-data.com API가 일시적으로 응답하지 않고 있습니다 (요청 과다 등).<br>'
        '이 지표들(MVRV, NUPL, 실현가격 등)은 잠시 후 새로고침하면 자동으로 표시됩니다.<br>'
        '<span style="color:#8b7500;font-size:0.78rem;">'
        '💡 참고: 같은 소스의 SOPR·Puell·MVRV Z-Score는 아래 "고급 온체인 지표" 섹션에 표시됩니다.</span>'
        '</div>',
        unsafe_allow_html=True,
    )
